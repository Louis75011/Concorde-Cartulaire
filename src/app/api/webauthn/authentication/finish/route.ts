...
export async function POST(req: Request) {
  ...
  const [pk] = await db
    ...
    .where(
      and(
        eq(user_passkeys.user_id, Number(uid)),
        eq(user_passkeys.credential_id, credIdB64url)
      )
    )
    .limit(1);
  if (!pk)
  ...
  try {
    ...
    await db
      ...
      .where(
        and(
          eq(user_passkeys.user_id, Number(uid)),
          eq(user_passkeys.credential_id, pk.credential_id)
        )
      );
    ...
    console.error("[WEBAUTHN /authentication/finish] error:", e?.message || e);
    return NextResponse.json(
