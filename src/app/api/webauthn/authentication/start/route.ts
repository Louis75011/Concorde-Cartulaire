...
export async function POST(req: Request) {
  ...
  const passkeys = await db.select().from(user_passkeys).where(eq(user_passkeys.user_id, u.id));
  if (!passkeys.length) return NextResponse.json({ error: "no passkeys for user" }, { status: 404 });
  ...
  res.cookies.set("webauthn_auth_uid", String(u.id), {
